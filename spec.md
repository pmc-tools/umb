...
var Annotation = schema({  
  ...
  "?#strings": Number.min(1).max(2^32-1).step(1), // present iff the type of the annotation is "string" and indicates the number of strings used  
  "?probability-type": Type, // present iff that the annotation represents a distribution over values (as used in stochastic observations), with probabilities provided using this (continuous numeric) type  
};
